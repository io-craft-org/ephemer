...
@pytest.mark.django_db
def test_staff_can_see_other_people_session_details(client, mocker):
    def mock_get(self, endpoint, data={}):
        return {
            "REAL_WORLD_CURRENCY_CODE": "USD",
            "admin_url": "http://localhost:8001/SessionStartLinks/93r6k4ov",
...
@pytest.mark.django_db
def test_create_session_with_participant_count_exceeding_maximum(client):
    experiment = Recipe(models.Experiment, maximum_participant_count=10).make()
    data = {"name": "Test Session", "participant_count": 12}
    with login(client):
        response = client.post(
            reverse("experiments-session-create", args=(experiment.pk,)), data=data
